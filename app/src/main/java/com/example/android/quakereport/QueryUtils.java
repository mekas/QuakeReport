...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes(String urlstring) {
        ...
        try {
            ...
            JSONArray jsonarr = reader.optJSONArray("features");
            int length = jsonarr.length();
            for (int i = 0; i < length; i++) {
                JSONObject earthquake_json=jsonarr.getJSONObject(i);
                JSONObject properties = earthquake_json.getJSONObject("properties");
